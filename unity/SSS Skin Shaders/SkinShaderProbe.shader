Shader "Custom/Skin Shader Probe" {
    ...
    _BumpBias ("柔和法线", Range(0.0,8.0)) = 1.5 //ddx ddy
    _S3STR ("SSS Strenght", Float) = 1.5
    _S3SPower ("_S3SPower", Range(0,1)) = 1.0
...
//            fixed3 Gloss;
        ...
        float _BumpBias, _CurvatureScale, _Fresnel,_ProbePow,_ProbeShininess,_ProbeSpecStr,_ProbeSpecSub,_S3STR,_S3SPower;
        float4 _Color;
...
//            o.Albedo = albedo.rgb;
        ...
        inline fixed4 LightingSkinShader( SurfaceOutputSkinShader s, fixed3 lightDir, fixed3 viewDir, fixed atten ){
            ...
            fixed4 c;
            c.rgb = (lerp(s.Albedo * saturate(NdotL) * atten, s.Albedo * (brdf*_S3STR) * m, _S3SPower   ) * _LightColor0.rgb + (spec * m * _LightColor0.rgb) ) * 2;
            //c.rgb = (lerp(s.Albedo * saturate(NdotL) * atten, s.Albedo * (brdf*_S3STR) * m, _S3SPower * s.Specular.b ) * _LightColor0.rgb + (spec * m * _LightColor0.rgb) ) * 2;
